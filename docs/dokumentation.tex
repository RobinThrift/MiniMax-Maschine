...
Wir haben zwei Algorithmen implementiert, da bei dem ersten ursprünglcihen Algorithmus wurde der 1. Benchmark nicht eingelesen.\\
...
\item \textbf{alu\_use (alle verwendeten ALU-Befehle):} $2*6 = 12$ Befehle
\end{enumerate}

\subsubsection{Laufzeit}
Mit der Laufzeit sind hier die Takte, die die Minimax-Maschine braucht, um unser Ergebnis zu erzeugen, gemeint. Leider war es nicht möglich mit diesem Algorithmus den ersten Benchmark fehelerfrei zu behandeln.  $t_{bench}$ betr{\"a}gt in unserem Fall 3019 Takte für den zweiten Benchmark und <palceholder>\\

\textbf{Berechnung:} \\ 
Für den ersten Benchmark: $t_{bewertet} = 16201 * (1 + 0,1*7 + 0,015*6 + 0,05 * 7) = 5672,4$ Takte
Für den zweiten Benchmark: $t_{bewertet} = 3019 * (1 + 0,1*7 + 0,015*6 + 0,05 * 7) = 1048,4$ Takte\\

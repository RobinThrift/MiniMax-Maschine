...
Kopf mit einer L{\"a}nge von 80 Bits und einem Datenteil mit variabler L{\"a}nge. Der Kopf enth{\"a}lt die Kanalnummer zwischen der 32. und 47. Bitstelle. Zu einem Kanal können ein oder mehrere Pakete gehören, welche die selbe Kanalnummer haben.
...
Um eine Operation auszuführen müssen über die Multiplexer `ALUSel.A` und `AluSel.B` zwei Operanden ausgew{\"a}hlt werden
und der ALU muss über die `ALU Ctrl`-Leitung der Code für die Operation übergben werden. Das Ergebnis der Operation kann
entweder in einem Register oder im HS (Adresse im Register `MAR`) gespeichert werden. Zus{\"a}tzlich können s. g. Flags 
